...
type color = ElementType<typeof colors> | 'initial' | [number, number, number];

const mkColorSeq = (offset: number) => (color: color = 'initial') => {
  if (color === 'initial') return { csi: true, str: `${offset + 9}m` };
  if (Array.isArray(color)) return { csi: true, str: `${offset + 8};2;${color.join(';')}m` };
...
const mkProcessStr = (fn: (color?: color) => string) => (text: string, color?: color) => {
  return text.split('\n').map((line) => {
...
const colorProcessStr = mkProcessStr((color?: color) => Seq.escape(colorSeq(color)));
const bgColorProcessStr = mkProcessStr((color?: color) => Seq.escape(bgColorSeq(color)));
